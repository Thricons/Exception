...
public class Window
{
    ...
    public Window(final Module.Category category, final int x, final int y) {
        ...
        this.max = 120;
        int y2 = y + 25;
        for (final Module c : Exception.instance.moduleManager.moduleList) {
            if (c.getCategroy() != category) {
                continue;
            }
    ...
    public void render(final int mouseX, final int mouseY) {
        ...
        if (this.category.name().equals("Elements")) {
            FontLoaders.kiona16.drawString("Elements", this.x + 58, this.y + 6, new Color(220, 220, 220).getRGB());
        }
        if (this.expand > 0) {
