...
import cn.exception.event.events.EventRender3D;
...
public class NameTags extends Module {
    private Option armor = new Option("Armor",  true);
    private Option dura = new Option("Durability",  true);
    private Numbers scale = new Numbers<Double>("Scale",  3.0, 1.0, 5.0, 0.1);
    ...
    public NameTags(){
        ...
        this.addValues(this.armor, this.dura, this.invis, this.scale);
    }
    ...
    public void onRender(EventRender3D render){
        ...
        validEnt.forEach(player -> {
            float x = (float)(player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)render.getPartialTicks() - Wrapper.renderPosX());
            float y = (float)(player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)render.getPartialTicks() - Wrapper.renderPosY());
            float z = (float)(player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)render.getPartialTicks() - Wrapper.renderPosZ());
    ...
    private void drawNames(EntityPlayer player) {
        ...
        RenderUtil.drawRect(nw + 5.0f, -3.0f, width, 10.0f,  new Color(0, 0, 0, 40).getRGB());
        //BlurUtil.blurArea(5, -3.0f, width, 10.0f);
    ...
    private void startDrawing(float x, float y, float z, EntityPlayer player) {
        ...
        double size = Wrapper.zoomMode() ? (double)(this.getSize(player) / 10.0f) * (double) this.scale.getValue() * 0.5 : (double)(this.getSize(player) / 10.0f) * (double) this.scale.getValue() * 1.5;
        GL11.glPushMatrix();
